...
def add():
    ...
    prodotto_originale = data['prodotto']
    prodotto_visualizzato = data.get('prodotto_modificato', prodotto_originale) # Gestisce es: "Roku Tonic"
    nota = data.get('nota', '')
    ...
    for cat, cont in MENU.items():
        if (isinstance(cont, list) and prodotto_originale in cont) or (isinstance(cont, dict) and prodotto_originale in cont):
            categoria_prodotto = cat
            break

    prezzo_base = determina_prezzo_base(prodotto_originale)
    prezzo_finale = float(prezzo_pers) if (prezzo_pers is not None and prezzo_pers != "") else (3.0 if "Shot" in nota else prezzo_base)
    ...
    for _ in range(quantita):
        tavoli_stato[tavolo_num]["ordine"].append({
            "prodotto": prodotto_visualizzato, "note": nota, "prezzo": prezzo_finale, 
