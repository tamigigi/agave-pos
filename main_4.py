...
def add():
    ...
    # FIX MOLTIPLICATORE CALICI: Prepariamo la nota dei calici
    nota_con_calici = nota
    if calici > 0:
        nota_con_calici = f"{nota} | {calici} CALICI" if nota else f"{calici} CALICI"

    reparto_competenza = REPARTI_PRODOTTI.get(categoria_prodotto, "bar")
        
    for i in range(quantita):
        # Assegna la nota dei calici SOLO al primo elemento del ciclo
        nota_corrente = nota_con_calici if (calici > 0 and i == 0) else nota
        
        tavoli_stato[tavolo_num]["ordine"].append({
            "prodotto": nome_comanda, "note": nota_corrente, "prezzo": prezzo_finale, 
...
# --- ROTTE PER LA STAMPA (Sistema a Coda per il Cloud) ---
