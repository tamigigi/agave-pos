...
def determina_prezzo_base(prodotto):
    # 1. Se il prezzo è estratto direttamente dal nome del prodotto (Es: "Nuggets di Pollo (6.00€)")
    match = re.search(r'\((\d+\.\d+)€\)', prodotto)
    if match:
        return float(match.group(1))
        
    # 2. Controllo nei dizionari (Soft Drink, Vini)
    for cat, contenuto in MENU.items():
        if isinstance(contenuto, dict) and prodotto in contenuto: 
            return contenuto[prodotto]
...
def stampa(num):
    ...
    if dest != "cassa":
        print(f"[COPIA PER CONOSCENZA] Inviata alla CASSA (.204)")
